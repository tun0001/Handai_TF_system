...
if __name__ == "__main__":
    ...
    # 正規表現のテスト
    import re
    pattern = r'^([\u3000-\u303F\u3400-\u4DBF\u4E00-\u9FFF\u3040-\u309F\u30A0-\u30FF\s]+)'
    match = re.match(pattern, name)
    if match:
        print(f"正規表現マッチ: '{match.group(1)}'")
    else:
        print("正規表現マッチなし")
    ...
    for url in urls:
        run_real_time_v2(url=url, univ=univ_name, spread_sheet_ID_conference=spread_sheet_ID_conference, spread_sheet_ID_member=spread_sheet_ID_member, creds_dict=creds_dict,announce_discord=announce_discord)