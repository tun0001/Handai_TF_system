...
def parse_conference_title(html):
    ...
    if not title_table:
        ...
        raise ValueError("大会名が見つかりません: title tableが存在しません")
        
    title_h3 = title_table.find('h3')
    if not title_h3:
        raise ValueError("大会名が見つかりません: title tableにh3が存在しません")
    
    title_text = title_h3.get_text(separator=' ', strip=True)
    if not title_text:
